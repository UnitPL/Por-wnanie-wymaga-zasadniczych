...
# (c) Emanuil Rusev  —  MIT License
#

class AnnexDiffParsedown
{
    const version = '1.8.0';
    ...
    function setBreaksEnabled($breaksEnabled)   { $this->breaksEnabled = $breaksEnabled; return $this; }
    protected $breaksEnabled;

    function setMarkupEscaped($markupEscaped)   { $this->markupEscaped = $markupEscaped; return $this; }
    protected $markupEscaped;

    function setUrlsLinked($urlsLinked)         { $this->urlsLinked = $urlsLinked; return $this; }
    protected $urlsLinked = true;

    function setSafeMode($safeMode)             { $this->safeMode = (bool) $safeMode; return $this; }
    protected $safeMode;

    function setStrictMode($strictMode)         { $this->strictMode = (bool) $strictMode; return $this; }
    ...
    protected $BlockTypes = array(
        ...
        '0' => array('List'), '1' => array('List'), '2' => array('List'),
        '3' => array('List'), '4' => array('List'), '5' => array('List'),
        '6' => array('List'), '7' => array('List'), '8' => array('List'), '9' => array('List'),
        ':' => array('Table'),
    ...
    protected function blockComment($Line)
    {
        ...
        {
            $Block = array(
                'element' => array('rawHtml' => $Line['body'], 'autobreak' => true),
            );
    ...
    protected function blockFencedCode($Line)
    {
        ...
        $Block = array(
            ...
            'element' => array('name' => 'pre', 'element' => $Element),
        );
        return $Block;
    }
    ...
    protected function blockList($Line, ?array $CurrentBlock = null)
    {
        ...
        if (preg_match('/^(' . $pattern . '([ ]++|$))(.*+)/', $Line['text'], $matches))
        {
            ...
            $Block = array(
                ...
                'element' => array('name' => $name, 'elements' => array()),
            );
    ...
    protected function blockListContinue($Line, array $Block)
    {
        ...
        if ($Line['indent'] < $requiredIndent
            and (
                ($Block['data']['type'] === 'ol' and preg_match('/^[0-9]++'.$Block['data']['markerTypeRegex'].'(?:[ ]++(.*)|$)/', $Line['text'], $matches))
                or ($Block['data']['type'] === 'ul' and preg_match('/^'.$Block['data']['markerTypeRegex'].'(?:[ ]++(.*)|$)/', $Line['text'], $matches))
            )
        ) {
            if (isset($Block['interrupted']))
            {
    ...
    protected function blockRule($Line)
    {
        ...
        {
            return array('element' => array('name' => 'hr'));
        }
    }
    ...
    protected function blockMarkup($Line)
    {
        ...
        if (preg_match('/^<[\/]?+(\w*)(?:[ ]*+' . $this->regexHtmlAttribute . ')*+[ ]*+(\/)?>/', $Line['text'], $matches))
        {
            ...
            $Block = array(
                'name'    => $matches[1],
                'element' => array('rawHtml' => $Line['text'], 'autobreak' => true),
            );
            return $Block;
        }
    }
    ...
    protected function blockTable($Line, ?array $Block = null)
    {
        ...
        foreach ($headerCells as $index => $headerCell)
        {
            ...
            $HeaderElement = array(
                'name'    => 'th',
                'handler' => array('function' => 'lineElements', 'argument' => $headerCell, 'destination' => 'elements'),
            );
            if (isset($alignments[$index]))
            {
                $HeaderElement['attributes'] = array('style' => "text-align: {$alignments[$index]};");
            }
        ...
        $Block = array(
            ...
            'identified' => true,
            'element'    => array('name' => 'table', 'elements' => array()),
        );
    ...
    protected function blockTableContinue($Line, array $Block)
    {
        ...
        if (count($Block['alignments']) === 1 or $Line['text'][0] === '|' or strpos($Line['text'], '|'))
        {
            ...
            foreach ($cells as $index => $cell)
            {
                ...
                $Element = array(
                    'name'    => 'td',
                    'handler' => array('function' => 'lineElements', 'argument' => $cell, 'destination' => 'elements'),
                );
    ...
    protected function paragraph($Line)
    {
        return array(
            ...
            'element' => array(
                'name'    => 'p',
                'handler' => array('function' => 'lineElements', 'argument' => $Line['text'], 'destination' => 'elements'),
            ),
        );
    }
    ...
    protected function lineElements($text, $nonNestables = array())
    {
        ...
        {
            if ( ! isset($Element['autobreak'])) $Element['autobreak'] = false;
        }
    ...
    protected function inlineEmailTag($Excerpt)
    {
            ...
            and preg_match("/^<((mailto:)?$commonMarkEmail)>/i", $Excerpt['text'], $matches))
        {
            ...
            return array(
                'extent'  => strlen($matches[0]),
                'element' => array('name' => 'a', 'text' => $matches[1], 'attributes' => array('href' => $url)),
            );
        }
    }
    ...
    protected function inlineEmphasis($Excerpt)
    {
        ...
        return array(
            ...
            'element' => array(
                'name'    => $emphasis,
                'handler' => array('function' => 'lineElements', 'argument' => $matches[1], 'destination' => 'elements'),
            ),
        );
    }
    ...
    protected function inlineLink($Excerpt)
    {
        $Element = array(
            'name'         => 'a',
            'handler'      => array('function' => 'lineElements', 'argument' => null, 'destination' => 'elements'),
    ...
    protected function inlineMarkup($Excerpt)
    {
        ...
        {
            return array('element' => array('rawHtml' => $matches[0]), 'extent' => strlen($matches[0]));
        }
    ...
    protected function inlineUrl($Excerpt)
    {
            ...
            and preg_match('/\bhttps?+:[\/]{2}[^\s<]+\b\/*+/ui', $Excerpt['context'], $matches, PREG_OFFSET_CAPTURE))
        {
            ...
            return array(
                ...
                'element'  => array('name' => 'a', 'text' => $url, 'attributes' => array('href' => $url)),
            );
        }
    }
    ...
    {
        foreach ($Elements as &$Element) $Element = $this->elementApplyRecursive($closure, $Element);
        return $Elements;
    }

    protected function elementsApplyRecursiveDepthFirst($closure, array $Elements)
    {
        foreach ($Elements as &$Element) $Element = $this->elementApplyRecursiveDepthFirst($closure, $Element);
        return $Elements;
    }
    ...
    protected function element(array $Element)
    {
        ...
        if ($hasContent)
        {
            ...
            if (isset($Element['elements']))       $markup .= $this->elements($Element['elements']);
            elseif (isset($Element['element']))    $markup .= $this->element($Element['element']);
            else
            {
                if ( ! $permitRawHtml) $markup .= self::escape($text, true);
                else                   $markup .= $text;
            }
    ...
    protected function sanitiseElement(array $Element)
    {
        ...
        if ( ! empty($Element['attributes']))
        {
            foreach ($Element['attributes'] as $att => $val)
            {
                ...
                elseif (self::striAtStart($att, 'on'))       unset($Element['attributes'][$att]);
            }
        }
        return $Element;
    }
    ...
    protected function filterUnsafeUrlInAttribute(array $Element, $attribute)
    {
        ...
        {
            if (self::striAtStart($Element['attributes'][$attribute], $scheme)) return $Element;
        }
    ...
    protected $regexHtmlAttribute = '[a-zA-Z_:][a-zA-Z0-9:._-]*(?:\s*=\s*(?:[^"\'=<>`\s]+|\'[^\']*\'|"[^"]*"))?';
...
/**
 * Custom markdown renderer: 4-space-indented lines render as paragraphs,
 * not <pre><code> blocks (EU Annex legal text uses deep indentation for sub-items).
 */
class AnnexDiffMarkdown extends AnnexDiffParsedown
{
    protected function blockCode( $Line, $Block = null )
    {
        return null; // never treat 4-space-indented lines as <pre><code>
    }
